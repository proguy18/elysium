...
public class Item : MonoBehaviour {
	...
	public GameObject startingObject; // the object that the item is initially attached to

	private void Start(){
		startingObject = gameObject; 
	}

	public void modifyStats(float multiplier){
		float hpMod = multiplier*(1 + getGaussian());
		float damageMod = multiplier*(1 + getGaussian());
		float armourMod = multiplier*(1 + getGaussian());
		maxHealthModifier = Convert.ToInt32(Math.Ceiling(maxHealthModifier*hpMod));
		damageModifier = Convert.ToInt32(Math.Ceiling(damageMod*damageModifier));
